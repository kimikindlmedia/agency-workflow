...
import { ContentTypeBadge, TagBadge } from '../components/Badge.jsx'
...
function InspirationModal({ isOpen, onClose, item = null }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEdit) {
      dispatch({ type: 'UPDATE_INSPIRATION', payload: { id: item.id, updates: payload } })
    } else {
      dispatch({ type: 'ADD_INSPIRATION', payload })
    }
    handleClose()
  }
...
function InspirationCard({ item }) {
  ...
  const hasMedia = item.fileData || item.url
...
export default function InspirationPage() {
  ...
  }, [state.inspiration, typeFilter, search])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Inspirace</h1>
          <p className="text-sm text-gray-500 mt-0.5">{state.inspiration.length} položek</p>
        </div>
      ...
      {state.inspiration.length > 0 && (
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-1 overflow-x-auto">
            ...
            >
              Vše ({state.inspiration.length})
            </button>
            {TYPE_OPTIONS.map(opt => {
              const count = state.inspiration.filter(i => i.type === opt.value).length
      ...
      {filtered.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-1">
            {search || typeFilter !== 'all' ? 'Žádná inspirace nenalezena' : 'Zatím žádná inspirace'}
          </h3>
          <p className="text-gray-500 text-sm">
            {search || typeFilter !== 'all' ? 'Zkuste jiný filtr nebo hledaný výraz.' : 'Přidejte první inspiraci kliknutím na tlačítko výše.'}
          </p>
        </div>
      ) : (
      ...
      <InspirationModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} />
    </div>
  )
}
